...
imagem_sopro_fogo = carregar_imagem("sopro (1.png", (150, 80))  # Imagem do sopro de fogo
...
# Distância mínima para o sopro
DISTANCIA_SOPRO = 120  # Distância mínima para o dragão soprar o fogo
FORCA_SOPRO = 200  # Força do sopro (quanto o jogador será empurrado)
...
while rodando:
    ...
    if not dragao_parado:
        # Verifica a distância entre o dragão e o jogador
        distancia = jogador.centerx - dragao.centerx
        if abs(distancia) <= DISTANCIA_SOPRO:
            # O dragão está perto o suficiente para usar o sopro
            imagem_dragao = imagem_dragao_ataque  # Muda para imagem de ataque do dragão
            temporizador_ataque_dragao = 0  # Reiniciar o temporizador de ataque
            # Empurrar o jogador para longe na direção oposta
            if distancia > 0:
                jogador.x += FORCA_SOPRO  # Empurrando para a direita
            else:
                jogador.x -= FORCA_SOPRO  # Empurrando para a esquerda
            # Desenha o sopro de fogo
            tela.blit(imagem_sopro_fogo, (dragao.centerx + 20, dragao.centery - 40))  # Ajuste da posição do sopro
        else:
            # Caso contrário, o dragão faz o movimento normal
            dragao.y += velocidade_dragao * direcao_dragao
            if dragao.top <= 100 or dragao.bottom >= 550:
                direcao_dragao *= -1

            # Dispara a bola de fogo normalmente
    ...
    tela.blit(fonte.render(f'XP: {xp_jogador}', False, branco), (30, 150))

    tela.blit(imagem_dragao, (dragao.x, dragao.y))
    if bola_fogo:
        tela.blit(imagem_bola_fogo, (bola_fogo.x, bola_fogo.y))
    if sopro_fogo:
        tela.blit(imagem_sopro_fogo, (bola_fogo.x, bola_fogo.y))
